...
def pivot_months_pandas(data):
    ...
    grouped_monthly = data.groupby(['name', 'month'])
    grouped_monthly = grouped_monthly.aggregate('sum').reset_index()
    monthly = grouped_monthly.pivot(index='name', columns='month', values='precipitation')
    # print(monthly)

    grouped_count = data.groupby(['name', 'month'])
    grouped_count = grouped_count.aggregate('count').reset_index()
