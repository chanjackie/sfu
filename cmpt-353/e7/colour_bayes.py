...
def main(infile):
    ...
    print("Training score: %g, Validation score: %g" % (model_rgb.score(X_train, y_train), model_rgb.score(X_test, y_test)))

    # TODO: build model_lab to predict y from X by converting to LAB colour first.
    transformer = sklearn.preprocessing.FunctionTransformer(color.rgb2lab)
    model_lab = make_pipeline(transformer, GaussianNB())

    model_lab.fit(X_train, y_train)

    # TODO: print model_lab's accuracy score
    print("Training score: %g, Validation score: %g" % (model_lab.score(X_train, y_train), model_lab.score(X_test, y_test)))
