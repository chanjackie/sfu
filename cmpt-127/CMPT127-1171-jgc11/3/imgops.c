	       ...
	       unsigned int rows )
{
  ...
  for (int i=0;i<rows;i+=2) {
    for (int n=0;n<cols;n+=2) {
      double one = array[n+i*cols];
      double two = array[n+i*cols+1];
      double three = array[n+(i+1)*cols];
      double four = array[n+(i+1)*cols+1];
      double average = round((one+two+three+four)/4);
      half[newIndex] = (uint8_t)average;
      newIndex++;
    }
  }
  return half;
