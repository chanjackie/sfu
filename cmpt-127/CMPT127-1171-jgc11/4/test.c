...
int main( int argc, char* argv[] )
{
  ...
  //scale_brightness(img,imgwidth,imgheight,3);
  ...
  draw_shutdown();
  free(halfimg);*/
  //region_set(img,imgwidth,imgheight,100,100,350,300,150);
  ...
  draw_shutdown();
  free(copy);*/
  //draw_circle(img, 400, 400, 50, 50, 25, 255);
  int testw = 383;
  int testh = 257;
  draw_rectangle(img,testw,testh,276,310,15,-89,122);
  /*for (int i=0;i<testh;i++) {
    for (int n=0;n<testw;n++) {
      printf("%d",img[n+i*testw]);
    }
    printf("\n");
  }*/
  ...
  for( int i=0; i<5; i++ )
    set_pixel( img, testw, testh, glider[i][0], glider[i][1], 255 );

  for( int i=0; i<32; i++ )
    {
      draw_image_grey( img, testw, testh );
      life( img, testw, testh );
    }*/
  draw_image_grey( img, testw, testh );
