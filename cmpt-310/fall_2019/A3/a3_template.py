...
class HMM():
	...
	def viterbi(self, sequence):
		...
		for n in range(self.num_states):
			for k in range(self.num_states):
				# prob = self.log_sum([(self.prior[k]), (self.transition[k][n]), (self.emission[k][sequence[0]])])
		...
		for t in range(1, len(sequence)):
			for i in range(self.num_states):
				for j in range(self.num_states):
					# prob = M[t-1][j] + self.log_sum([self.transition[j][i], self.emission[j][sequence[t]]])
	...
	def posterior(self, sequence):
		###########################################
		# Start your code
		forwardProbs = matrix(len(sequence), self.num_states)
		for i in range(self.num_states):
			forwardProbs[0][i] = self.prior[0]
		print(forwardProbs)
		# End your code
		###########################################
